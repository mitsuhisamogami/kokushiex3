...
- タイトルは `[add]`・`[fix]`・`[chore]` など Git 履歴と同じ角括弧ラベル + 要約で統一し、`Closes #123` の形式で関連 Issue を明記します。
- `gh pr create` を利用する場合でも `--body` でテンプレート内容を反映し、`docker-compose exec web bundle exec rspec` など実行したテストコマンドを備考に記載します。マイグレーションやシード変更がある場合は手順も添えます。
