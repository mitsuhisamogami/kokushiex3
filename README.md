...
```bash
# 本番環境向け: マイグレーション + 本番用シード
docker-compose exec web rails db:prepare:production

# 開発環境向け: マイグレーション + 本番用シード + 開発専用シード
docker-compose exec web rails db:prepare:development
```

本番に投入する試験・問題・選択肢・合格基準・タグの seed は `db/fixtures/production` に配置します。
開発用ユーザー、ダミー受験履歴、ダミー回答、スコアは `db/fixtures/development` にのみ配置します。
開発環境を初期化する場合は `db:prepare:development` を実行してください。
...
- 実行内容:
  ...
  - `run_seed=true` 指定時は `db:prepare:production` で本番用 seed も投入
